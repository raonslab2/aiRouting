...
4) 백엔드: Freerouting CLI로 라우팅/검증 → SES/log를 base64로 반환(현재는 파일 저장만, 적용은 추후).
5) 응답을 패널 로그에 표시하고 임시 출력 폴더에 SES/log를 저장.
...
## TODO
- SES 제안/적용 흐름: 현재는 SES/log를 저장만 함. 이후 KiCad에 직접 적용(pcbnew API/세션 재적용) 기능 추가.
